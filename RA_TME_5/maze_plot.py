...
def plot_points(points, maze="maze_hard.pbm", title=None):
    x,y = zip(*points)
...
def plot_points_lists(lpoints, maze="maze_hard.pbm", title=None):
    fig1, ax1 = plt.subplots()
    ax1.set_xlim(0,600)
    ax1.set_ylim(600,0) # Decreasing
    ax1.set_aspect('equal')
...
def plot_points_file(filename, maze="maze_hard.pbm", title=None):
    try:
